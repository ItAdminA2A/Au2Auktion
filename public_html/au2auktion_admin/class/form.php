<?php
	...
	class form
	{
		...
		function html()
		{
			...
			{
				// Tab header 1
				$tmp1 = new tpl("_form_tab_header_1");
				...
				for ($i = 0; $i < count($this->tabs); $i++)
				{
					...
					{
						$tmp1 = new tpl("_form_tab_header_tab_error");
					}
					else
					{
						$tmp1 = new tpl("_form_tab_header_tab");
					}
					$tmp1->set("id", $i);
				...
				// Tab header 2
				$tmp1 = new tpl("_form_tab_header_2");
				$tmp1->set("form_id", $this->form_id);
				$tmp->add("tab", $tmp1->html());
			}
			$html .= $tmp->html();
			...
			for ($i = 0; $i < count($this->fields); $i++)
			{
				...
				if ($this->fields[$i]["type"] == "input")
				{
					...
					if ($show_confirm)
					{
						$tmp = new tpl("_form_confirm");
					}
					else
					{
						$tmp = new tpl("_form_input");
					}
				}
				elseif ($this->fields[$i]["type"] == "input2")
				{
					...
					{
						$tmp = new tpl("_form_input2");
					}
				}
				elseif ($this->fields[$i]["type"] == "readonly")
				{
					// Readonly
					$tmp = new tpl("_form_readonly");
				}
				elseif ($this->fields[$i]["type"] == "textarea")
				{
					...
					else
					{
						...
						if ($this->fields[$i]["wysiwyg"])
						{
							...
							$tmp = new tpl("_form_textarea_wysiwyg");
						...
						elseif ($this->fields[$i]["popup_editor"] != false)
						{
							...
							{
								// Regel editor
								$tmp = new tpl("_form_textarea_popup_rules");
							}
							...
							{
								// Wysiwyg editor
								$tmp = new tpl("_form_textarea_popup_wysiwyg");
								$tmp->set("css", $this->wysiwyg_css);
							}
						}
						else
						{
							$tmp = new tpl("_form_textarea");
						}
					}
				}
				elseif ($this->fields[$i]["type"] == "password")
				{
					...
					if ($show_confirm)
					{
						$tmp = new tpl("_form_confirm_password");
						$tmp->set("value_star", str_repeat("*", strlen($this->fields[$i]["value"])));
					}
					else
					{
						$tmp = new tpl("_form_password");
					}
				}
				elseif ($this->fields[$i]["type"] == "radio")
				{
					...
					else
					{
						...
						for ($i2 = 0; $i2 < count($this->fields[$i]["options"]); $i2++)
						{
							...
							if ($value == $this->fields[$i]["value"])
							{
								$tmp = new tpl("_form_radio_option_selected");
							}
							else
							{
								$tmp = new tpl("_form_radio_option");
							}
							$tmp->set("i", $i2);
				...
				elseif ($this->fields[$i]["type"] == "select")
				{
					...
					else
					{
						...
						for ($i2 = 0; $i2 < count($this->fields[$i]["options"]); $i2++)
						{
							...
							if ($value == $this->fields[$i]["value"])
							{
								$tmp = new tpl("_form_select_option_selected");
							}
							else
							{
								$tmp = new tpl("_form_select_option");
							}
							$tmp->set("value", $value);
							$tmp->set("option", $option);
							$options .= $tmp->html();
						}
						$tmp = new tpl("_form_select");
						$tmp->set("options", $options);
					}
				}
				elseif ($this->fields[$i]["type"] == "combo")
				{
					...
					else
					{
						...
						{
							list($value, $option) = $this->fields[$i]["options"][$i2];
							$tmp = new tpl("_form_combo_option");
							$tmp->set("value", $value);
							$tmp->set("option", $option);
							$options .= $tmp->html();
						}
						$tmp = new tpl("_form_combo");
						$tmp->set("options", $options);
					}
				}
				elseif ($this->fields[$i]["type"] == "checkbox")
				{
					...
					if ($show_confirm)
					{
						...
						{
							$tmp = new tpl("_form_confirm_true");
						}
						else
						{
							$tmp = new tpl("_form_confirm_false");
						}
					}
					else
					{
						if ($this->fields[$i]["value"] == 1 or $this->fields[$i]["value"])
						{
							$tmp = new tpl("_form_checkbox_checked");
						}
						else
						{
							$tmp = new tpl("_form_checkbox");
						}
					}
				}
				...
				{
					// Template-visning
					$tmp = new tpl("_form_tpl_" . $this->fields[$i]["name"]);
				...
				elseif ($this->fields[$i]["type"] == "image")
				{
					...
					if ($show_confirm)
					{
						...
						{
							$tmp = new tpl("_form_confirm_image");
							$tmp->set("time", time());
						}
						else
						{
					...
					else
					{
						if (is_file($_document_root . $this->fields[$i]["value"]))
						{
							...
							$tmp = new tpl("_form_image_example");
							$tmp->set("time", time());
						}
						else
						{
							// Viser upload-boks
							$tmp = new tpl("_form_image");
						}					
					}
				}
				elseif ($this->fields[$i]["type"] == "file")
				{
					...
					if ($show_confirm)
					{
						if (is_file($_document_root . $this->fields[$i]["value"]))
						{
							$tmp = new tpl("_form_confirm_file");
					...
					else
					{
						...
						{
							// Viser fil
							$tmp = new tpl("_form_file_example");
							$tmp->set("size", number_format(filesize($_document_root . $this->fields[$i]["value"]) / 1024, 1, ",", "."));
							$tmp->set("realname", $this->values[$this->fields[$i]["name"] . "_realname"]);
						}
						else
				...
				elseif ($this->fields[$i]["type"] == "tab")
				{
					...
					if ($has_tab)
					{
						...
						$tmp = new tpl("_form_tab_div_footer");
						$html .= $tmp->html();
					}
					$tmp = new tpl("_form_tab_div_header");
					$tmp->set("form_id", $this->form_id);
					$has_tab = true;
				}
				...
				{
					// Ny kolonne
					$tmp = new tpl("_form_new_column");
				}
				else
				{
					// Hidden
					$tmp = new tpl("_form_hidden");
				}
			...
			if ($show_confirm)
			{
				$tmp = new tpl("_form_footer_confirm");
			}
			else
			{
				$tmp = new tpl("_form_footer");
			}
