<?php
	...
	function OBA_numberselect_html($from, $to, $value = "", $interval = 1, $k_seperator = true)
	{
		...
		{
			$html .= "<option value=\"$i\" " . ($value == $i ? "selected" : "") . ">" . number_format($i, 0, ",", $k_seperator ? "." : "") . "</option>";
	...
	function OBA_fpdf_layout(&$fpdf, $title, $left, $right1, $right2, $invoice_lines = false)
	{
		...
		$fpdf->Image($_document_root . "/layouts/Standard/img/logo.png", 140, 10, 55);
	...
	function OBA_no()
	{
		...
		$db = new db;
		$db->execute("LOCK TABLES " . $_table_prefix . "_settings_module WRITE");
		$id = intval(module_setting("no")) + 1;
		module_setting("no", $id);
		$db->execute("UNLOCK TABLES");
		return $id;
	}

	function OBA_invoice_no()
	{
		global $_table_prefix, $module;
		$db = new db;
		$db->execute("LOCK TABLES " . $_table_prefix . "_settings_module WRITE");
		$id = intval(module_setting("invoice_no")) + 1;
		module_setting("invoice_no", $id);
		$db->execute("UNLOCK TABLES");
		return $id;
	}

	function OBA_auction_no($id)
	{
		global $_table_prefix, $module;
		$db = new db;
		$db->execute("
			...
			WHERE
				id = '" . $db->escape($id) . "'
			");
		if (!$res = $db->fetch_array()) return false;
		...
		{
			// Fysisk
			$db->execute("LOCK TABLES " . $_table_prefix . "_module_" . $module . "_auctions WRITE");
			$auction_no = intval($db->execute_field("
				SELECT
					MAX(auction_no)
				FROM
					" . $_table_prefix . "_module_" . $module . "_auctions
	...
	$array_paid_method = array(
		"" => "",
		"cash" => "Kontant",
		"card" => "Betalingskort",
		"bank" => "Bank"
		);
	$select_paid_method = array();
	foreach ($array_paid_method as $key => $val) $select_paid_method[] = array($key, $val);